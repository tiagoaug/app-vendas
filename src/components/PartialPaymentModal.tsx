...
export default function PartialPaymentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onPay(val, accountId, note);
      setAmount('');
      setNote('');
      // Keep open if user wants to pay more? Or close? User said "quantos pagamentos necessários", better close after each to show updated list.
      onClose();
    } catch (error) {
      console.error(error);
    } finally {
          ...
          {viewMode === 'PAYMENT' ? (
            ...
            <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
              <div className={`p-5 rounded-3xl flex items-center justify-between ${isDarkMode ? 'bg-rose-900/20' : 'bg-rose-50'}`}>
                <div>
                  <p className="text-[9px] font-black text-rose-500 uppercase tracking-widest mb-1">Restante a Pagar</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 mb-2 block">Conta Bancária</label>
                    <div className="relative">
                ...
                >
                  {isSubmitting ? 'Processando...' : <><CheckCircle2 size={18} strokeWidth={3} /> Registrar Pagamento</>}
                </button>
              </form>
            </div>
          ) : (
            ...
            <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-300">
              <div className={`p-5 rounded-3xl flex items-center justify-between ${isDarkMode ? 'bg-emerald-900/20' : 'bg-emerald-50'}`}>
                <div>
                  <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-1">Status da Dívida</p>
              ...
              <div className={`rounded-3xl p-5 border ${isDarkMode ? 'bg-slate-950/50 border-slate-800' : 'bg-slate-50/50 border-slate-100'}`}>
                ...
                {purchase.paymentHistory && purchase.paymentHistory.length > 0 ? (
                      ...
                      return (
                        <div key={p.id} className="flex items-center justify-between border-b last:border-0 border-slate-50 dark:border-slate-900 pb-3 last:pb-0">
                          ...
                          <div className="text-right">
                             <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{acc?.name}</p>
                  ...
                  <div className="text-center py-10">
                    <p className="text-[10px] font-black uppercase text-slate-300 dark:text-slate-700 tracking-widest italic">Nenhum pagamento realizado</p>
                  </div>
                )}
              </div>
            </div>
